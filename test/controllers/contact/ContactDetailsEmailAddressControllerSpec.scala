...
class ContactDetailsEmailAddressControllerSpec extends ControllerSpec with DefaultAwaitTimeout {
  ...
  "ContactDetailsEmailAddressController" should {
    ...
    "return 303 (OK) for get status throw error" when {
      "user submits an email address" in {
        ...
        spyJourneyAction.setReg(reg)
        mockRegistrationUpdate()
        mockEmailVerificationGetStatusWithException(DownstreamServiceError("Failed to get status", new Exception()))

        intercept[DownstreamServiceError] {
          await(controller.submit()(postRequestEncoded(EmailAddress("test@test.com"))))
        }
        reset(mockRegistrationConnector)
  ...
  "return an error" when {
    ...
    "user submits form and the registration update fails" in {
      ...
      intercept[DownstreamServiceError](status(controller.submit()(postRequestEncoded(EmailAddress("test@test.com")))))
    }
