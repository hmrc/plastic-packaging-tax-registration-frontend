...
class AddGroupMemberConfirmBusinessAddressControllerSpec
  ...
  ".displayPage" should {
    ...
    "display business address for confirmation when it is populated and valid" in {
      ...
      val reg =
        aRegistration(withGroupDetail(Some(GroupDetail(Some(false), Seq(aGroupMember().copy(id = testMemberId))))))
