...
class ReviewRegistrationViewSpec extends UnitViewSpec with Matchers with TableDrivenPropertyChecks {
  ...
  "Review registration View" should {
    ...
    "display title" in {
      view.getElementsByClass(gdsPageHeading).first() must containMessage("reviewRegistration.organisationDetails.title")
    }

    "display labels, values and change links" when {

      def getKeyFor(section: Int, index: Int, view: Document = view) =
        view.getElementsByClass("govuk-summary-list").get(section).getElementsByClass("govuk-summary-list__key").get(index)

      def getValueFor(section: Int, index: Int, view: Document = view) =
        view.getElementsByClass("govuk-summary-list").get(section).getElementsByClass("govuk-summary-list__value").get(index).text()
                  ...
                  GroupDetail(
                    ...
                    members = Seq(
                      GroupMember(
                        ...
                        organisationDetails = Some(GroupMemberOrganisationDetails("UkCompany", "Subsidiary 1", Some("XP00123"))),
                        contactDetails = Some(
                          GroupMemberContactDetails(
                            ...
                            address = Some(
                              Address(addressLine1 = "1", addressLine2 = None, addressLine3 = None, townOrCity = "New Street", maybePostcode = Some("AB12CD"), countryCode = GB)
                            )
                          )
                        ),
                        addressDetails =
                      ...
                      GroupMember(
                        ...
                        organisationDetails = Some(GroupMemberOrganisationDetails("UkCompany", "Subsidiary 2", Some("XP00456"))),
                        contactDetails = Some(
                          GroupMemberContactDetails(
                            firstName = "Test2",
                            lastName = "User2",
                            ...
                            address = Some(
                              Address(addressLine1 = "2", addressLine2 = None, addressLine3 = None, townOrCity = "New Street", maybePostcode = Some("AB12CE"), countryCode = GB)
                            )
                          )
                        ),
                        addressDetails =
                          Address(addressLine1 = "2", addressLine2 = None, addressLine3 = None, townOrCity = "New Street", maybePostcode = Some("AB12CD"), countryCode = GB)
                      )
                    )
                  )
                )
        ...
        s"registering a $registrationType" when {
          ...
          "displaying liability details section" when {
            ...
            "already exceeded threshold" in {
              ...
              getKeyFor(liabilitySection, 0, liabilityView) must containMessage("liability.checkAnswers.exceededThreshold")
              getKeyFor(liabilitySection, 1, liabilityView) must containMessage("liability.checkAnswers.dateExceededThreshold")
              ...
              getChangeLinkFor(liabilitySection, 0, liabilityView) must haveHref(controllers.liability.routes.ExceededThresholdWeightController.displayPage.url)
              getChangeLinkFor(liabilitySection, 1, liabilityView) must haveHref(liabilityRoutes.ExceededThresholdWeightDateController.displayPage())
            }
            ...
            "not already exceeded threshold but expect to in next 30d" in {
              ...
              getKeyFor(liabilitySection, 0, liabilityView) must containMessage("liability.checkAnswers.expectToExceededThreshold")
              getKeyFor(liabilitySection, 1, liabilityView) must containMessage("liability.checkAnswers.dateRealisedExpectToExceededThreshold")
              getKeyFor(liabilitySection, 2, liabilityView) must containMessage("liability.checkAnswers.exceededThreshold")
              ...
              getChangeLinkFor(liabilitySection, 0, liabilityView) must haveHref(liabilityRoutes.ExpectToExceedThresholdWeightController.displayPage().url)
              getChangeLinkFor(liabilitySection, 1, liabilityView) must haveHref(liabilityRoutes.ExpectToExceedThresholdWeightDateController.displayPage)
              getChangeLinkFor(liabilitySection, 2, liabilityView) must haveHref(liabilityRoutes.ExceededThresholdWeightController.displayPage)
            }
          ...
          "displaying organisation details section" when {
            ...
            "registering uk company" in {
              ...
              getKeyFor(organisationSection, 0, ukCompanyView) must containMessage("reviewRegistration.organisationDetails.organisationType")
              getKeyFor(organisationSection, 1, ukCompanyView) must containMessage("reviewRegistration.organisationDetails.organisationName")
              getKeyFor(organisationSection, 2, ukCompanyView) must containMessage("reviewRegistration.organisationDetails.businessRegistrationNumber")
              getKeyFor(organisationSection, 3, ukCompanyView) must containMessage("reviewRegistration.organisationDetails.uniqueTaxpayerReference")
              getKeyFor(organisationSection, 4, ukCompanyView) must containMessage("reviewRegistration.organisationDetails.registeredBusinessAddress")
              ...
              getValueFor(organisationSection, 2, ukCompanyView) mustBe ukCompanyRegistration.organisationDetails.incorporationDetails.get.companyNumber
              getValueFor(organisationSection, 3, ukCompanyView) mustBe ukCompanyRegistration.organisationDetails.incorporationDetails.get.ctutr.get
              getValueFor(organisationSection, 4, ukCompanyView) mustBe "2 Scala Street Soho London W1T 2HN United Kingdom"
            ...
            "registering sole trader" in {
              ...
              getKeyFor(organisationSection, 0, soleTraderView) must containMessage("reviewRegistration.organisationDetails.organisationType")
              getKeyFor(organisationSection, 1, soleTraderView) must containMessage("reviewRegistration.organisationDetails.soleTrader.firstName")
              getKeyFor(organisationSection, 2, soleTraderView) must containMessage("reviewRegistration.organisationDetails.soleTrader.lastName")
              getKeyFor(organisationSection, 3, soleTraderView) must containMessage("reviewRegistration.organisationDetails.soleTrader.dob")
              getKeyFor(organisationSection, 4, soleTraderView) must containMessage("reviewRegistration.organisationDetails.soleTrader.nino")
              getKeyFor(organisationSection, 5, soleTraderView) must containMessage("reviewRegistration.organisationDetails.registeredBusinessAddress")

              getValueFor(organisationSection, 0, soleTraderView) mustBe OrgType.displayName(OrgType.SOLE_TRADER)
              getValueFor(organisationSection, 1, soleTraderView) mustBe soleTraderRegistration.organisationDetails.soleTraderDetails.get.firstName
              getValueFor(organisationSection, 2, soleTraderView) mustBe soleTraderRegistration.organisationDetails.soleTraderDetails.get.lastName
              getValueFor(organisationSection, 3, soleTraderView) mustBe soleTraderRegistration.organisationDetails.soleTraderDetails.get.dateOfBirth.get
              getValueFor(organisationSection, 4, soleTraderView) mustBe soleTraderRegistration.organisationDetails.soleTraderDetails.get.ninoOrTrn
              getValueFor(organisationSection, 5, soleTraderView) mustBe "2 Scala Street Soho London W1T 2HN United Kingdom"
            ...
            "registering partnership" in {
              ...
              getKeyFor(organisationSection, 0, partnershipView) must containMessage("reviewRegistration.organisationDetails.organisationType")
              getKeyFor(organisationSection, 1, partnershipView) must containMessage("reviewRegistration.organisationDetails.partnership.name")
              getKeyFor(organisationSection, 2, partnershipView) must containMessage("reviewRegistration.organisationDetails.registeredBusinessAddress")

              getValueFor(organisationSection, 0, partnershipView) mustBe PARTNERSHIP.toString
              getValueFor(organisationSection, 1, partnershipView) mustBe generalPartnershipDetailsWithPartners.partnershipOrCompanyName.get
              getValueFor(organisationSection, 2, partnershipView) mustBe "2 Scala Street Soho London W1T 2HN United Kingdom"
            }
          ...
          "displaying primary contact details section" in {
            ...
            getChangeLinkFor(contactDetailsSection, 0) must haveHref(contactRoutes.ContactDetailsFullNameController.displayPage())
            getChangeLinkFor(contactDetailsSection, 1) must haveHref(contactRoutes.ContactDetailsJobTitleController.displayPage())
            getChangeLinkFor(contactDetailsSection, 2) must haveHref(contactRoutes.ContactDetailsEmailAddressController.displayPage())
            getChangeLinkFor(contactDetailsSection, 3) must haveHref(contactRoutes.ContactDetailsTelephoneNumberController.displayPage())
            getChangeLinkFor(contactDetailsSection, 4) must haveHref(contactRoutes.ContactDetailsConfirmAddressController.displayPage())
          }
          ...
          "displaying group member details" in {
            ...
            registrationType match {
              case SINGLE_ENTITY =>
                groupMembersView.getElementsByClass("govuk-summary-list").size mustBe 3 // Group member section not displayed!
              case GROUP =>
                registration.groupDetail.get.members.zipWithIndex.foreach {
                  case (member, idx) =>
                    groupMembersView.getElementById(s"group-members-heading-${idx + 1}").text() must include(member.businessName)

                    val groupMemberRows =
                      groupMembersView.select(s"div#group-members-content-${idx + 1} dl div")

                    verifyRowContent(groupMemberRows.get(0), messages("reviewRegistration.member.orgName"), member.organisationDetails.get.organisationName, None)
                    verifyRowContent(
                    ...
                    verifyRowContent(groupMemberRows.get(2), messages("reviewRegistration.member.companyNumber"), member.customerIdentification1, None)
                    verifyRowContent(groupMemberRows.get(3), messages("reviewRegistration.member.utr"), member.customerIdentification2.get, None)
                    verifyRowContent(
          ...
          "displaying nominated partner details" in {
            ...
            getKeyFor(nominatedPartnerSection, 0, partnershipView) must containMessage("reviewRegistration.partner.orgType")
            getKeyFor(nominatedPartnerSection, 1, partnershipView) must containMessage("reviewRegistration.partner.name")
            getKeyFor(nominatedPartnerSection, 2, partnershipView) must containMessage("reviewRegistration.partner.dob")
            getKeyFor(nominatedPartnerSection, 3, partnershipView) must containMessage("reviewRegistration.partner.nino")
            getKeyFor(nominatedPartnerSection, 4, partnershipView) must containMessage("reviewRegistration.partner.utr")
            getKeyFor(nominatedPartnerSection, 5, partnershipView) must containMessage("reviewRegistration.partner.contact.name")
            getKeyFor(nominatedPartnerSection, 6, partnershipView) must containMessage("reviewRegistration.partner.contact.jobTitle")
            getKeyFor(nominatedPartnerSection, 7, partnershipView) must containMessage("reviewRegistration.partner.contact.email")
            getKeyFor(nominatedPartnerSection, 8, partnershipView) must containMessage("reviewRegistration.partner.contact.phone")
            getKeyFor(nominatedPartnerSection, 9, partnershipView) must containMessage("reviewRegistration.partner.contact.address")
            val nominatedPartner = partnershipRegistration.nominatedPartner.get
            getValueFor(nominatedPartnerSection, 0, partnershipView) mustBe PartnerTypeEnum.displayName(nominatedPartner.partnerType)
            getValueFor(nominatedPartnerSection, 1, partnershipView) mustBe nominatedPartner.name
            getValueFor(nominatedPartnerSection, 2, partnershipView) mustBe nominatedPartner.soleTraderDetails.get.dateOfBirth.get
            getValueFor(nominatedPartnerSection, 3, partnershipView) mustBe nominatedPartner.soleTraderDetails.get.ninoOrTrn
            getValueFor(nominatedPartnerSection, 4, partnershipView) mustBe nominatedPartner.soleTraderDetails.get.sautr.get
            getValueFor(nominatedPartnerSection, 5, partnershipView) mustBe nominatedPartner.contactDetails.get.name.get
            getValueFor(nominatedPartnerSection, 6, partnershipView) mustBe nominatedPartner.contactDetails.get.jobTitle.get
            getValueFor(nominatedPartnerSection, 7, partnershipView) mustBe nominatedPartner.contactDetails.get.emailAddress.get
            getValueFor(nominatedPartnerSection, 8, partnershipView) mustBe nominatedPartner.contactDetails.get.phoneNumber.get
            getValueFor(nominatedPartnerSection, 9, partnershipView) mustBe "1 High Street Cloverfield Leeds LS1 1AA United Kingdom"
          ...
          "displaying other partner details" in {
            ...
            partnershipRegistration.otherPartners.zipWithIndex.foreach {
              case (partner, idx) =>
                ...
                verifyRowContent(otherPartnerRows.get(0), messages("reviewRegistration.partner.orgType"), PartnerTypeEnum.displayName(partner.partnerType), None)
                verifyRowContent(otherPartnerRows.get(1), messages("reviewRegistration.partner.companyNumber"), partner.incorporationDetails.get.companyNumber, None)
                verifyRowContent(
