        ...
        "display organisation details" in {
          ...
          val expectedOrganisationDetails =
            Seq(
              ...
              registration.organisationDetails.businessRegisteredAddress.map(bra =>
                bra.maybePostcode.getOrElse("")
              ).get,
              countryService.tryLookupCountryName(
                registration.organisationDetails.businessRegisteredAddress.map(bra => bra.countryCode).get
              )
            ).filter(_.nonEmpty)
          ...
          organisationType match {
            case Partnership =>
              ...
              changeLinks.get(0) must haveHref(
                partnerAmendRoutes.AmendPartnerContactDetailsController.contactName(nominatedPartner.id)
              )
              changeLinks.get(1) must haveHref(
                partnerAmendRoutes.AmendPartnerContactDetailsController.jobTitle(nominatedPartner.id)
              )
              changeLinks.get(2) must haveHref(
                partnerAmendRoutes.AmendPartnerContactDetailsController.emailAddress(nominatedPartner.id)
              )
              changeLinks.get(3) must haveHref(
                partnerAmendRoutes.AmendPartnerContactDetailsController.phoneNumber(nominatedPartner.id)
              )
              changeLinks.get(4) must haveHref(
                partnerAmendRoutes.AmendPartnerContactDetailsController.address(nominatedPartner.id)
              )
            case _ =>
