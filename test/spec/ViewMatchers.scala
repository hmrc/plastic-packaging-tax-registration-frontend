...
trait ViewMatchers {
  ...
  implicit class PageComplexChecks(document: Document) extends MustMatchers {
    ...
    def checkErrorsSummary(): Unit = {
      ...
      document.select("div.error-summary.error-summary--show>p").text() must be(
        "error.summary.text"
      )
    }
  ...
  def containErrorElementWithMessageKey(
    key: String
  )(implicit messages: Messages): Matcher[Element] =
  ...
  def containMessageForElements(key: String, args: Any*)(implicit
    messages: Messages
  ): Matcher[Elements] =
  ...
  def haveFieldError(fieldName: String, content: String): Matcher[Element] =
    new ContainElementWithIDMatcher(
      s"error-message-$fieldName-input"
    ) and new ElementContainsFieldError(fieldName, content)
