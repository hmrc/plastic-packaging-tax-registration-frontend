...
trait MockConnectors extends MockitoSugar with RegistrationBuilder with BeforeAndAfterEach {
  ...
  def mockGetSoleTraderDetails(soleTraderDetails: SoleTraderDetails): OngoingStubbing[Future[SoleTraderDetails]] =
  ...
  def mockGetPartnershipDetailsFailure(ex: Exception): OngoingStubbing[Future[PartnershipBusinessDetails]] =
    when(mockPartnershipGrsConnector.getDetails(any())(any(), any()))
      .thenThrow(ex)
  ...
  def mockCreatePartnershipGrsJourneyCreation(redirectUrl: String): OngoingStubbing[Future[String]] =
    when(
  ...
  def mockCreateSoleTraderPartnershipGrsJourneyCreation(redirectUrl: String): OngoingStubbing[Future[String]] =
    when(
      mockSoleTraderGrsConnector.createJourney(any[SoleTraderGrsCreateRequest], any[String])(any(), any())
    ).thenReturn(Future.successful(redirectUrl))

  def mockCreateUkCompanyPartnershipGrsJourneyCreation(redirectUrl: String): OngoingStubbing[Future[String]] =
    when(
  ...
  def mockRegisteredSocietyCreateIncorpJourneyId(redirectUrl: String): OngoingStubbing[Future[String]] =
