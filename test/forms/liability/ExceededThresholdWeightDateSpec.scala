  ...
  "ExceededThresholdWeightDate" should {
    ...
    "error correctly" when {
      ...
      "only two date fields is present" in {
        val boundForm =
          sut.bind(Map("exceeded-threshold-weight-date.day" -> "15", "exceeded-threshold-weight-date.month" -> "5"))
      ...
      "date is before go live date" in {
        ...
        boundForm.errors mustBe Seq(
          FormError("exceeded-threshold-weight-date.day", Seq(isBeforeLiveDateError), Seq("1 some message 2022"))
        )
      }
