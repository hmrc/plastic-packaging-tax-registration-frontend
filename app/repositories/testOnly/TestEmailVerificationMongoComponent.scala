...
@Singleton
class TestEmailVerificationMongoComponent @Inject()(
  ...
  private val mongoUri =
    configuration.getOptional[String]("emailVerificationTestMongodb.uri") match {
      case Some(uri) => uri
      case _ => configuration.get[String]("mongodb.uri")
    }
